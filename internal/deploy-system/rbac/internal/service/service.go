...
// 创建一个角色
func (s *Service) CreateRole(ctx context.Context, command *domain.CreateRoleCommand) (id types.Long, err error) {
	/*
		开启事务
	*/
	ctx, err = s.BeginTransaction(ctx, "role service create")
	if err != nil {
		return
	}
	/*
	...
	*/
	defer func() {
		err = s.FinishTransaction(ctx, err, "role service create")
	}()
...
// 根据ID修改角色
func (s *Service) ModifyRoleByID(ctx context.Context, command *domain.ModifyRoleCommand) (err error) {
	/*
		开启事务
	*/
	ctx, err = s.BeginTransaction(ctx, "role service ModifyRoleByID")
	if err != nil {
		return
	}
	defer func() {
		err = s.FinishTransaction(ctx, err, "role service ModifyRoleByID")
	}()
...
func (s *Service) DeleteRoleByID(ctx context.Context, ID types.Long) (err error) {
	/*
		开启事务
	*/
	ctx, err = s.BeginTransaction(ctx, "role service DeleteRoleByID")
	if err != nil {
		return
	}
	defer func() {
		err = s.FinishTransaction(ctx, err, "role service DeleteRoleByID")
	}()
